...
// Store API base URL in localStorage so the user can connect to port 8000 or production domains
const STORAGE_KEY = 'yazaki_api_base_url_v2';

export function getApiBaseUrl(): string {
  const saved = localStorage.getItem(STORAGE_KEY);
  if (saved) return saved;
  // If we are developing locally on port 3000, we check if FastAPI is running on default port 8000
  // Defaulting to empty string "" makes it fetch relative to current origin, which is standard for Cloud Run deployment!
  return '';
}
...
export const api = {
  ...
  // 4. Export invoice to Excel (returns raw Blob object or trigger direct download)
  ...
  // 7. Reset Plant department budget
  ...
  // 8. Adjust Plant department budget
  ...
  // 9. Get Config file representation
  async getConfig(): Promise<ConfigResponse> {
    return request<ConfigResponse>('/api/config');
  },

  // 10. Update the whole configuration
  ...
  // 11. Add client to plant mapping
  ...
  // 15. Get transactions audit history
